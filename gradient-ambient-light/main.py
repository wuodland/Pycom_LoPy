...
def calculateColor(value, uplimit, downlimit):
    if value > uplimit:
        value = uplimit
    elif value < downlimit:
        value = downlimit
    
    ratio = (value - downlimit)/(uplimit - downlimit)
    ...
    val1 = int((x3 - r3)/x3 * 255)
    val2 = int(r3/x3 * 255)

    color = "0x%02X00%02X" % (val1, val2)
...
while True:
    ...
    color = calculateColor(LuxValue, 100, 20)
    pycom.rgbled(color)
